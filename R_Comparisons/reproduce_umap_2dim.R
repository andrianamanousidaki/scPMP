...
for(d in dataset){
  for(m in method){
    print(d)
    res<-clustering(dataset=d,method=m,geometric_perturbation=TRUE,
                    two_dimensions=TRUE)
    accuracy<-as.data.frame(res$accuracy)
    runtime<-as.data.frame(res[4:5])
...
saveRDS(acc_res,paste0('../Results/reproduce_',method,'2dims.rds'))

library(writexl)
write_xlsx(acc_res,paste0('../Results/reproduce_',method,'2dims.xlsx'))

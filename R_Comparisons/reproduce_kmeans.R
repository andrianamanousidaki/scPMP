...
for(d in dataset){
  for(m in method){
    print(d)
    res<-clustering(dataset=d,method=m,geometric_perturbation=FALSE,
                    two_dimensions=FALSE)
    accuracy<-as.data.frame(res$accuracy)
    complete_rt<-res$runtime$complete_rt
    clustering_rt<-res$runtime$clustering_rt
    method<-res$method
    nclust<-length(unique(res$labels))
    result<-cbind(accuracy,method,complete_rt,clustering_rt,d,nclust)
    acc_res<-rbind(acc_res,result)
  }
  
}

saveRDS(acc_res,paste0('../Results/reproduce_',method,'mdims.rds'))

library(writexl)
write_xlsx(acc_res,paste0('../Results/reproduce_',method,'mdims.xlsx'))

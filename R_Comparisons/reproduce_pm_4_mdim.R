...
for(d in dataset){
  for(m in method){
    print(d)
    res<-clustering(dataset=d,method=m,p=4,geometric_perturbation=TRUE,
...
saveRDS(acc_res,paste0('../Results/reproduce_',method,'_4_mdims.rds'))

library(writexl)
write_xlsx(acc_res,paste0('../Results/reproduce_',method,'_4_mdims.xlsx'))

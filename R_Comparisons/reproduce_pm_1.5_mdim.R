...
for(d in dataset){
  for(m in method){
    print(d)
    res<-clustering(dataset=d,method=m,p=1.5,geometric_perturbation=TRUE,
                    two_dimensions=FALSE, predict_pm_clusters=FALSE)
...
saveRDS(acc_res,paste0('../Results/reproduce_',method,'_1.5_mdims.rds'))

library(writexl)
write_xlsx(acc_res,paste0('../Results/reproduce_',method,'_1.5_mdims.xlsx'))
